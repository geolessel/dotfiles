...
(use-package heex-ts-mode
  :straight t
  :hook (heex-ts-mode . (lambda () (add-hook
                                    'before-save-hook
...
(use-package elixir-ts-mode
  :straight t
  :hook (elixir-ts-mode . (lambda () (add-hook
                                   'before-save-hook
                                   'geo/elixir-format-buffer
                                   nil
                                   t))))
...
(use-package lsp-ui
  :straight t
  :bind
  (("C-s-<return>" . lsp-ui-doc-focus-frame))
  :config
  (add-hook 'lsp-ui-doc-frame-hook 'evil-normal-state)
  (define-key evil-normal-state-map (kbd "M-RET") 'lsp-ui-doc-glance)
  (define-key evil-normal-state-map (kbd "C-M-<return>")
              #'(lambda ()
                  (interactive)
                  (lsp-ui-doc-toggle)
                  (message "C-s-<return> to focus in documentation")))
...
#+begin_src emacs-lisp
(defun geo/toggle-lsp-variable (variable)
  "Toggle a VARIABLE related to LSP UI Sideline"
  (interactive)
  (set variable (not (symbol-value variable)))
  (message "%S is now %s" variable (symbol-value variable)))

(pretty-hydra-define geo/hydra-lsp (:exit t :quit-key "q")
  ("Sideline"
   (("s d" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-sideline-show-diagnostics)) "Show diagnostics in sideline")
    ("s h" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-sideline-show-hover)) "Show hover in sideline")
    ("s a" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-sideline-show-code-actions)) "Show actions in sideline"))
   "Peek"
   (("p d" lsp-ui-peek-find-definitions "Find definitions")
    ("p r" lsp-ui-peek-find-references "Find references")
    ("p /" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-peek-show-directory)) "Show the directory"))
   "Doc"
   (("d e" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-doc-enable)) "Enable/disable")
    ("d c" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-doc-show-with-cursor)) "Show docs with cursor")
    ("d m" (lambda () (interactive) (geo/toggle-lsp-variable 'lsp-ui-doc-show-with-mouse)) "Show docs with mouse"))
   "imenu"
   (("i s" lsp-ui-imenu "Show imenu"))
))
#+end_src
...
*** ChatGPT / LLMs
They may take my job someday but for now they work for me.

#+begin_src emacs-lisp
(pretty-hydra-define geo/hydra-llm-menu (:quit-key "q")
  ("Chat/LLMs"
   (("c" geo/chatgpt-chat-with-code "Chat about a file or region")
    ("C" geo/chatgpt-get-input "Start a chat with ChatGPT"))))
#+end_src
...
(pretty-hydra-define geo/hydra-top-menu
  (:title "The world's your oyster"
    ...
    ("Working"
     ...
     "AI"
     (("c" geo/chatgpt-get-input "Ask ChatGPT anything")
      ("C" geo/chatgpt-chat-with-code "Ask ChatGPT about code"))
