...
To get crisper font rendering in macOS, run this in the command line:
...
** =use-package=
use-package is now built in to Emacs. Don't install other versions.
...
*** Use =elpaca= unless otherwise specified
#+begin_src emacs-lisp
...
*** Transient
Make sure that transient is loaded early so magit will use it and not
the built-in verison.

See https://github.com/magit/magit/issues/5329
#+begin_src emacs-lisp
...
When running Emacs in a terminal inside tmux (=emacs -nw= or
=emacsclient -t=), several things need special handling: true color,
clipboard, mouse, keybindings, and performance.

*** Help Emacs recognize the tmux-256color TERM
Emacs doesn't have a built-in terminfo entry for =tmux-256color=, so
we alias it to =xterm-256color= which Emacs does know.
...
OSC 52 lets terminal Emacs copy text to the system clipboard through
tmux. This works even over SSH. Emacs 29+ has built-in support.
...
*** Terminal performance tweaks
These reduce redisplay overhead in terminal mode where rendering is
inherently slower than GUI.
...
When reattaching a tmux session, environment variables like
=SSH_AUTH_SOCK= and =DISPLAY= go stale. This function pulls fresh
values from tmux.

#+begin_src emacs-lisp
(defun geo/refresh-tmux-env ()
  "Update Emacs environment from tmux session."
  (interactive)
  (when (getenv "TMUX")
    (let ((pairs (shell-command-to-string "tmux show-environment")))
      (dolist (line (split-string pairs "\n" t))
        (cond
         ((string-prefix-p "-" line)
          (setenv (substring line 1) nil))
         ((string-match "\\`\\([^=]+\\)=\\(.*\\)\\'" line)
...
I want some breathing space between the text and the frame, so let's
bump up that value a bit.
...
[[https://github.com/motform/stimmung-themes][stimmung-themes]] is a pretty minimal theme in regards to font
colors. It uses background colors and bold/italics to do a lot of its
communication.
...
;; There are tons of options to define for these.
...
;; Recommendation from Jesse Anderson
...
(setq modus-themes-bold-constructs t ; defualt nil
      ...
      '((1 . (overline background variable-pitch 1.3))
        ...
        (t . (semibold))))
#+end_src
*** Custom Catppuccin flavor switcher
The ~catppuccin~ theme comes with 4 "flavors" but there is no easy way
to switch between them. I want one! So I'll make one.

#+begin_src emacs-lisp
(defun geo/catppuccin-load-flavor (flavor)
...
want them. I wrote a minor mode to support PragmataPro specifically
(based off of code provided by the PragmataPro author himself at
https://github.com/fabrizioschiavi/pragmatapro/tree/master/emacs_snippets.

#+begin_src emacs-lisp
(when (display-graphic-p)
  (require 'pragmata-mode)
  (add-hook 'prog-mode-hook #'pragmata-mode)
  (add-hook 'text-mode-hook #'pragmata-mode)
  (add-hook 'conf-mode-hook #'pragmata-mode))
#+end_src

The author of PragmataPro maintains a script that helps set up
ligatures: [[https://github.com/fabrizioschiavi/pragmatapro/blob/master/emacs_snippets/][https://github.com/fabrizioschiavi/pragmatapro/blob/master/emacs_snippets/]]

#+begin_src emacs-lisp :tangle no
(load (concat user-emacs-directory "packages/" "pragmatapro-prettify-symbols-v0.829.el"))
(add-hook 'prog-mode-hook #'prettify-hook)
(add-hook 'text-mode-hook #'prettify-hook)
(add-hook 'conf-mode-hook #'prettify-hook)
(global-prettify-symbols-mode t)
#+end_src

I came across [[https://github.com/lumiknit/emacs-pragmatapro-ligatures][pragmatapro-lig]] specifically for enabling PragmataPro
font ligatures and I used to use it but it started throwing errors
when formatting with lsp-mode. I'm disabling for now in favor of the
above.
...
The ~pragmatapro-lig~ package has been giving me trouble lately and
when it is not giving me trouble, tends to slow down text
rendering. I'm attempting to use ~ligature~ instead (even though it
requires a lot more config).

#+begin_src emacs-lisp :tangle no
(use-package ligature :tangle no
  :config
  ;; 't to use these ligatures in every major mode
...
~evil-mode~ overrides ~TAB~ with ~evil-jump-forward~; restore
~org-cycle~ and made the changes required to do the same for terminal emacs.

#+begin_src emacs-lisp
(add-hook 'org-mode-hook
          (lambda ()
            (evil-local-set-key 'normal (kbd "<tab>") #'org-cycle)
            ;; In terminal mode, TAB is sent as C-i
            (evil-local-set-key 'normal (kbd "TAB") #'org-cycle)))
#+end_src
...
*** Increase imenu headline depth
When using =imenu= (or more likely =consult-imenu=) to search org
files, I'd like to search deeper.

#+begin_src emacs-lisp
(setq org-imenu-depth 4)
#+end_src
*** Try to fix heading folding
This was suggested on Reddit. It's driving me crazy. I'll try
anything. https://www.reddit.com/r/emacs/comments/1ds3aoy/anyone_else_occasionally_experience_being_unable/

#+begin_src emacs-lisp
(setq org-fold-core-style 'overlays)
(setq org-element-use-cache 0)
#+end_src
** Evil
...
(use-package typescript-mode
  :mode "\\.tsx?$"
  :hook (typescript-mode . (lambda () (setq flycheck-checker 'javascript-eslint)))
  :init
...
(use-package elixir-ts-mode
  :custom
  (lsp-elixir-server-command '("/Users/geo/src/lexical/_build/dev/package/lexical/bin/start_lexical.sh"))
  :hook ((elixir-ts-mode . eglot-ensure)
         (elixir-ts-mode . (lambda () (add-hook
                                       'before-save-hook
                                       'geo/elixir-format-buffer
                                       nil
                                       t)))))
...
** Ediff
Change some of the defaults of the built-in ~ediff~ package.

Most of this comes from [[https://www.youtube.com/watch?v=pSvsAutseO0][Prot's video on ediff]].
...
Automatically format c files on save with =clang-format=. I have a
copy of clang.format.el from https://github.com/llvm-mirror/clang/blob/master/tools/clang-format/clang-format.el

#+begin_src emacs-lisp
(require 'clang-format)

(defun geo/format-c-code-buffer ()
  "Format buffer of c or c++ code"
  (interactive)
  (when (or (derived-mode-p 'c-ts-mode)
            (derived-mode-p 'c++-ts-mode))
    (clang-format-buffer)))

(add-hook 'c-ts-mode-hook
          (lambda ()
            (add-hook 'before-save-hook 'geo/format-c-code-buffer nil t)))
(add-hook 'c++-ts-mode-hook
          (lambda ()
            (add-hook 'before-save-hook 'geo/format-c-code-buffer nil t)))
...
*** Treesitter markdown-ts-mode

Use markdown-ts-mode. Currently disabled.
...
*** Use native syntax coloring for code blocks

The code blocks must be specified but if they are, this will make it
so that they are colored or highlighted as if the block was itself
rendered in the major mode of the code in the code block.
...
#+BEGIN_SRC emacs-lisp
  (use-package diff-hl
    :config
    (global-diff-hl-mode)
    ;; Terminal Emacs has no fringe — use the margin instead
    (unless (display-graphic-p)
      (diff-hl-margin-mode)))
#+END_SRC
...
** Undo
Emacs' built-in undo system is pretty powerful but a little hard to
understand. There are other undo packages that dumb it down at the
expense of functionality, but *undo-tree* tries to make that power
come easier, especially with tree visualization.

The documentation for this one (with examples) is in the source code.

- [[https://www.dr-qubit.org/undo-tree.html][Homepage]]

Update: while I rolled with ~undo-tree~ for a while, I never really
used its more advanced features. Furthermore, I think it was never
quite set up correctly. I'm going to use ~undo-fu~ for a while which
relies more on the built-in undo functionality, just making it simpler
and act more like I'd expect.
...
tools specific to the language I'm writing. To that end, [[https://github.com/jscheid/prettier.el][prettier]] will
...
*** eglot
eglot is now built-in to Emacs, so let's use that instead of ~lsp-mode~.
...
;;   :config
;;   (define-key evil-normal-state-map (kbd "C-f") #'treesit-fold-toggle))
#+end_src
...
** tree-sitter
I compile my own Emacs with =--with-tree-sitter= which enables the new
tree-sitter functionality built into Emacs 29. In order to use
grammars, we need to do a couple of things:

The [[https://www.masteringemacs.org/article/how-to-get-started-tree-sitter][Mastering Emacs tree-sitter]] article provided a lot of the how-to
for getting this working.

*** Specify where grammars are located

#+begin_src emacs-lisp
(setq treesit-language-source-alist
      '(
        ;; provided by the official tree-sitter github org
        ...
        ;; other community-provided grammars
...
(defun geo/install-all-treesit-grammars ()
  "Install all grammars defined in `treesit-language-source-alist'."
  (interactive)
  (mapc #'treesit-install-language-grammar (mapcar #'car treesit-language-source-alist)))
#+end_src

*** Map old modes to new tree-sitter powered modes

Emacs will still default to the original non-tree-sitter
modes. However, we can get Emacs 29+ to remap one major mode to
another. We'll do this for each major mode we want to override with
its tree-sitter version.
...
(setq major-mode-remap-alist
      '((c-mode . c-ts-mode)
        ...
        ;; erlang-mode not defined
        ;; gitignore-mode not defined
        ;; make-mode not defined
        ;; (markdown-mode . markdown-ts-mode)
        ;; (org-mode . org-ts-mode)
        ;; racket-mode not defined
        (yaml-mode . yaml-ts-mode)
        ;; zig-mode not defined
        ))
#+end_src

*** Automatically install and use tree-sitter modes
=treesit-auto= automatically installs missing tree-sitter grammars and
remaps major modes to their tree-sitter equivalents when available.
...
*** Homegrown
Integrate with OpenAI ChatGPT. My own creation. Well, technically, I used ChatGPT to bootstrap it and add features.
...
*** GPTel
When I previously checked this package out before I started writing
my own, it didn't quite fit my use cases. It has been updated many
times since then and I think I can use it regularly now. Plus, I don't
have to maintain it!
...
(setq savehist-additional-variables
      ;; By default, the built-in `savehist-mode' only keeps a
      ;; record of minibuffer histories.  With the variable
      ;; `savehist-additional-variables' we can make `savehist-mode'
      ;; keep a record of any variable we want, so that it persists
      ;; between Emacs sessions. Let's also store the `kill-ring'
      ;; and the `register-alist'.
...
*** Save the bookmark file immediately
This snippet is [[https://protesilaos.com/codelog/2023-06-28-emacs-mark-register-basics/][from Prot]].
...
> to write to the disk as a final step before closing Emacs.

#+begin_src emacs-lisp
(defun geo/bookmark-save-no-prompt (&rest _)
  "Run `bookmark-save' without prompts.

The intent of this function is to be added as an :after advice to
`bookmark-set-internal'. This means that when
`bookmark-set-internal' is called, this function is called right
afterwards. We set this up because there is no hook after setting
a bookmark and we want to automatically save bookmarks at that
point."
  (funcall 'bookmark-save))
(advice-add 'bookmark-set-internal :after 'geo/bookmark-save-no-prompt)
#+end_src
...
For something new, I'm going to use ~consult-line~ instead.
...
(with-eval-after-load 'evil
  ...
  ;; In terminal mode, C-/ is sent as C-_ (both map to \x1f)
  (define-key evil-normal-state-map (kbd "C-_") 'geo/consult-ripgrep))
#+end_src
...
** Running `mix sobelow`
#+begin_src emacs-lisp
(defun geo/mix-sobelow ()
  "Run `mix sobelow` in the project root and display the output in a buffer."
  (interactive)
  (let* ((default-directory (locate-dominating-file (project-root (project-current)) "mix.exs"))
         (buffer (get-buffer-create "*sobelow*"))
         (command "mix sobelow"))
    (with-current-buffer buffer
      ...
      (let ((exit-code (call-process-shell-command command nil buffer t)))
        (if (eq exit-code 0)
            (progn
              (ansi-color-apply-on-region (point-min) (point-max)) ;; strip ansi colors
              (geo/sobelow--prepend-file-paths)
              (compilation-mode) ;; make the filepaths linkable
              (goto-char (point-min)))
          (message "Error running `mix sobelow`: exit code %d" exit-code))))
    (display-buffer buffer)))

(defun geo/sobelow--prepend-file-paths ()
  "Prepend umbrella app file paths in sobelow's output for current buffer."
  (let ((current-prefix ""))
    (goto-char (point-min))  ;; Go to the beginning of the buffer
    (while (not (eobp))  ;; Loop until the end of buffer
      (let ((line (thing-at-point 'line t)))
        (cond
         ;; Check for ==> VARIABLE
         ((string-match "^==> \\(.*\\)$" line)
          (setq current-prefix (concat "apps/" (match-string 1 line) "/")))
         ;; Check for File: PATH
         ((string-match "^File: \\(.*\\)$" line)
          (let ((original-path (match-string 1 line)))
            (delete-region (line-beginning-position) (line-end-position)) ;; Delete the old line
            (insert (concat "File: " current-prefix original-path))))) ;; Insert new line with prepended path
        (forward-line 1)))))  ;; Move to the next line

;; Match sobelow output and make the filenames and lines links to the exact spot
;;
;; sobelow output:
;;
;; File: apps/...
;; Line: 83
;; (add-to-list 'compilation-error-regexp-alist 'elixir-sobelow)
    ...
    ("Local"
     ...
     "External"
      (("g" consult-grep "Grep")
       ...
       ("R" geo/consult-ripgrep-thing-at-point "Ripgrep word at point")
  ...
  (pretty-hydra-define geo/hydra-buffer-menu (:exit t :quit-key "q")
    ("Buffers"
      (("b" consult-buffer "Switch (globally)")
       ...
       ("k" kill-current-buffer "Kill this buffer")
...
***** Show paren style
Cycle through the available options for =show-paren-style=.

#+begin_src emacs-lisp
(defun geo/toggle-show-paren-style ()
  "Cycle the value of show-paren-style."
  (interactive)
  (setq show-paren-style
        (cond
         ((eq show-paren-style 'parenthesis) 'expression)
         ((eq show-paren-style 'expression) 'mixed)
         ((eq show-paren-style 'mixed) 'parenthesis)
         (t 'expression))) ; default
  (message "show-paren-style set to %s" show-paren-style))
#+end_src

***** Show current local or UTC time
When displaying the time in the mode line, cycle through displaying
the time in local time and then in UTC time.

#+begin_src emacs-lisp
(defun geo/display-time-mode-display-utc ()
  "Set display-time-mode on and in UTC."
  (setq display-time-format (format-time-string "%H:%MZ" (current-time) t)
        geo/display-time-mode-format 'utc)
  (display-time-mode 1)
  (message "Displaying UTC time"))

(defun geo/display-time-mode-display-local ()
  "Set display-time-mode on and in local time zone."
  (setq display-time-format (format-time-string "%H:%M")
        geo/display-time-mode-format 'local)
  (display-time-mode 1)
  (message "Displaying local time"))

(defun geo/cycle-display-time-mode (&optional value)
  "Cycle display-time-mode and whether to show time in UTC or local zone.

VALUE can be one of -1, 'utc, or 'local. If -1, turn
display-time-mode off. Otherwise, turn the display on and set it
to the appropriate time zone."
  (interactive)
  (cond
   ((or (eq value -1)
        (and (not value) display-time-mode (eq geo/display-time-mode-format 'utc)))
    (display-time-mode -1)
    (message "Turning off time display"))
   ((or (eq value 'local)
        (and (not value) (not display-time-mode)))
    (geo/display-time-mode-display-local))
   ((or (eq value 'utc)
        (and (not value) display-time-mode (eq geo/display-time-mode-format 'local)))
    (geo/display-time-mode-display-utc))))

(geo/cycle-display-time-mode -1)
#+end_src

**** Internal frame padding
Add the ability to change the internal frame padding. Usually I like a
little bit of breathing room but sometimes that should be reduce to
fit in more text.

#+begin_src emacs-lisp
(defun geo/set-internal-frame-padding (pixels)
...
**** Line spacing
Speaking of text breathing room, sometimes having a bit of line
spacing is handy. While the process for changing this is pretty
simple, having a function in my hydra helps me remember that it can be
done in the first place.

#+begin_src emacs-lisp
(defun geo/set-line-spacing (spacing)
  "Set the line height globally and buffer-locally.

If value is a floating point number, it specifies the spacing
relative to the default frame line height. A value of 0 means
add no extra space."
  (interactive
   (let ((spacing (read-number "Spacing: " line-spacing)))
     (list spacing)))
  ...
  (pretty-hydra-define geo/hydra-prefs-menu (:quit-key "q")
    ("Display"
     ...
     (("d" diff-hl-mode "diff-hl" :toggle t)
      ...
      ("t" geo/toggle-show-trailing-whitespace "trailing whitespace" :toggle show-trailing-whitespace)
      ("p" geo/set-internal-frame-padding "set internal padding")
      ("l" geo/set-line-spacing "set line spacing"))
     "Editing"
     (("e" electric-pair-mode "electric-pair" :toggle t)
      ("f" auto-fill-mode "auto-fill" :toggle auto-fill-function))
     "Modeline"
     (("m c" column-number-mode "column number" :toggle t)
      ...
      ("m t" geo/cycle-display-time-mode "display time" :toggle display-time-mode)))))
#+end_src
...
*** AI
Menu for working with AI models directly.

#+begin_src emacs-lisp
(with-eval-after-load 'pretty-hydra
  (pretty-hydra-define geo/hydra-ai-menu (:quit-key "q")
    ("GPTel"
     (("g" gptel-send "Chat with buffer contents up to point")
      ("G" gptel-menu "Open gptel config menu")
      ("a" gptel-add "Add/remove region or buffer to context")
      ("f" gptel-add-file "Add/remove file to context")
      ("c" gptel "Create a new chat buffer")))))
#+end_src
*** Main
    ...
    (:title "The world's your oyster"
      ...
      ("Working"
       (("w" geo/hydra-workspace-menu/body "Workspaces")
        ...
        ("a" geo/hydra-ai-menu/body "AI"))

       "Getting Help"
       (("A" geo/hydra-apropos-menu/body "Apropos")
        ("l" geo/hydra-lsp/body "LSP"))
